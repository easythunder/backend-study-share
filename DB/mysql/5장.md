...
Key Point!- MVCC와 LOCK의 관계

MVCC는 읽기(SELECT)를 위한 동시성 기술이고,
락(Lock)은 쓰기(UPDATE, DELETE, INSERT) 충돌을 막기 위한 기술이다.
둘은 목적이 다르고 서로를 대체할 수 없습니다.
...
엔진 락은 MySQL 전체 또는 테이블 등 큰 범위를 잠그는 락이고,
레코드락과 갭락은 InnoDB 스토리지 엔진 내부에서 행 또는 범위 수준을 잠그는 락입니다.
---
