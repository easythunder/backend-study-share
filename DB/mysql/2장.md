...
- Question? 데이터 파일 구조의 어떤 부분이 달라서 조금씩 올리나요?
- Answer! 
    MySQL은 데이터를 16KB 페이지 조각으로 저장을 합니다.
    버전이 바뀌어도 기본 구조는 거의 그대로 유지가 되어서 예전 버전도 읽어 올 수는 있어요!
    다만 페이지 안에 작은 플래그, 옵션, 헤더의 바이트 수가 변화해서 점진적으로 변화를 시킵니다

    실제로 5.1에서 5.6으로 변화할 때, 페이지 헤더에서 사용하던 바이트 의미가 바뀌었고
    5.6 이후에는 compressed page라는 구조가 새로 생겼어요

    이전에는 테이블 정보가 파일 기반 메타데이터(ibd + frm)으로 나눠서 저장했더라면
    금은 모든 메타데이터를 시스템 테이블 (mysql.dd)에 넣어서 관리합니다
    5.1에서 frm을 8.0의 데이터 테이블 기반으로 변환하는 로직이 내부에 없어서
    중간에 5.7버전을 활용합니다 (5.7에는 변환 로직이 들어있어요)
