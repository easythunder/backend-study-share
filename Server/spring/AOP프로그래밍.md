...
<details><summary><code>💡 피드백</code> : 해보신 프로젝트가 있다면 예시를들어 설명해주세요</summary>

주류 리뷰 플랫폼에서 주류와 맛,향 정보가 N:M 관계로 하나의 요청에서 여러 테이블에 insert가 필요했습니다.  
데이터 정합성을 보장하기 위해 @transactional 을 사용했습니다.  
스프링은 프록시로 메서드 호출을 감싸 트랜잭션의 시작과 커밋, 롤백을 제어했습니다.  
그 결과 여러 insert를 하나의 트랜잭션 단위로 안전하게 처리할 수 있었습니다.  

</details>

<details><summary><code>💡 피드백</code> : 위빙이 발생하는 타이밍이 궁금해요</summary>

위빙은 컴파일 시점, 클래스로딩 시점, 런타임 시점에 발생할 수 있습니다.  
스프링 AOP는 프록시 기반으로 동작하며, 객체가 생성된 이후에 프록시를 통해 메서드 호출을 가로채는 방식이기 때문에 런타임 시점에 위빙이 발생합니다.  

</details>

<details><summary><code>💡 피드백</code> : Advice 실행 순서가 보장되지 않는다고 했는데 왜 보장되지 않나요?</summary>

여러 Aspect가 적용되면 스프링 AOP는 프록시 체인을 생성합니다.  
프록시 체인은 한번 생성되면 내부 순서는 고정되지만, 그 체인이 어떤 순서로 만들어질지는 빈 등록 순서 등에 따라 매번 달라질 수 있어서 명시하지 않으면 보장 되지 않습니다.  
